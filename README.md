...
op 模式 log 範例：
```
[INFO] [Space A | camera_a] Zone 0:2  Zone 1:1  FPS:30.0  skip:2
[WARN] [Space A | camera_a] ALERT: CROWD Zone 0: 5 persons
```
...
# 作業模式（無畫面，僅 log）
python src/run_space_a.py --cameras camera_a --mode op
...
│  camera_a        │  camera_b        │
│  [偵測畫面]      │  [偵測畫面]      │
│  People:2 FPS:30 │  People:1 FPS:29 │
└──────────────────┴──────────────────┘
         CSAS PoC — Space A
```
...
python src/run_space_b.py --mode op   # 作業模式
```
...
python src/run_space_c.py --mode op   # 作業模式
```
...
# 顯示確認
python3 -c "import cv2, numpy as np; img=np.zeros((480,640,3),dtype=np.uint8); cv2.rectangle(img,(100,100),(540,380),(0,255,0),-1); cv2.imshow('test',img); cv2.waitKey(3000); cv2.destroyAllWindows()"
# 即時 GPU 負載監控
tegrastats --interval 1000   # 推論中 GR3D_FREQ 應 > 0%
```
...
- [x] Jetson Orin TensorRT FP16 加速（yolo11n.engine，推論速度 3~5x）
- [x] Jetson GStreamer nvv4l2decoder H.265 硬體解碼（降低 CPU 負載）
- [x] dev / op 雙模式：開發者顯示畫面 / 作業模式無畫面純 log（op 模式 FPS ~30）
...
2. 重建 venv（見「Jetson 部署 步驟 1」），使用系統 OpenCV
3. 設定 SSH 顯示環境（見「Mac ↔ Jetson 快速切換」）
